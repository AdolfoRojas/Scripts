...
#bot <- Bot(token = "1739134259:AAFpIAj0ilBzSQDdhz7TXP3IkiiXMlMG_IE")
#chat_id <- 1525121917
#message_to_bot <- 'Script Iniciado:\n"P-Value Threshold y Anotación VEP"'
#bot$sendMessage(chat_id, text = message_to_bot)
#load('before_loop.RData')
#df <- read.delim("../Variant_info_EUR_filtered.tsv")
#df <- df[c("rsid","nearest_genes")]
#NCORES <- 10
...
################################################################################################
#print(paste("p =", p_value,"AUCinf =", AUC_inf[1], "SD =", AUC_inf[4], sep = " "))
#print(paste("p =", p_value,"AUCgrid-nosp =", AUC_pred_nosp[1], "SD =", AUC_pred_nosp[4], sep = " "))
#print(paste("p =", p_value,"AUCgrid-sp =", AUC_pred_sp[1], "SD =", AUC_pred_sp[4], sep = " "))
#print(paste("p =", p_value,"AUCauto =", AUC_Auto[1], "SD =", AUC_Auto[4], sep = " "))
system("rm tmp-data/*.sbk")
#non_zero_grid_sp <- info_snp[info_snp$best_grid_sp != 0,]
#df2 <- merge(non_zero_grid_sp, df, by = "rsid")
df2 <- merge(info_snp, df, by = "rsid")
...
write.table(DF3,file = paste("p-Value_threshold_", p_value, "_hapmap3_all_variant_effect_non_zero.txt", sep = ""), quote = F, col.names=F, row.names= F)
#system(paste("./ensembl-vep/vep --input_file ", paste("p-Value_threshold_", p_value, "_hapmap3_all_variant_effect_non_zero.txt", sep = ""), " --output_file ", paste("VEP_p-Value_threshold_", p_value, "_hapmap3_all_variant_effect_non_zero", sep = ""),
#" --cache --regulatory --no_headers --biotype --dir_cache ensembl-vep/cache/ --force_overwrite --verbose --per_gene --symbol --domains --fork 15 --offline --assembly GRCh37", sep = ""))
#system("rm *_warnings.txt")
#bot$sendPhoto(chat_id, photo = paste("Grid_model_plot_p-value_", p_value,".png", sep = ""))
#bot$sendPhoto(chat_id, photo = paste("Auto_model_plot_p-value_", p_value,".png", sep = ""))
...
ggdensity(prediccion_SP, x = "pred_sp",
            add = "mean", rug = TRUE,
            color = "V2", fill = "V2",
            palette = c("#00AFBB", "#E7B800"))+
    ggtitle("Distribution of test samples PRS", subtitle = paste("AUC Grid-SP = ", round(AUC_pred_sp[[1]],4), sep = ""))+ 
    theme(legend.position = c(0.85,0.75), plot.title = element_text(hjust = 0, size=20, vjust= 0), 
...
ggdotchart(prediccion_SP, x = "V1", y = "pred_sp",
           color = "V2",  
           dot.size = 0.5,                                         # Color by groups
           palette = c("#00AFBB", "#E7B800"), # Custom color palette
           sorting = "ascending",                        # Sort value in descending order
           ggtheme = theme_pubr()) + theme(axis.text.x=element_blank())+
