...
PieDonut(df_plot2,aes(pies=gender,donuts=race),showPieName=FALSE, title ="Distribucion segun raza", r0=0.2, r1 = 0.2, r2 = 0.4, start=5.9*pi/2, showRatioThreshold = getOption("PieDonut.showRatioThreshold", 0.0001), labelpositionThreshold=0.5, titlesize = 15, donutLabelSize = 6, addPieLabel = FALSE, pieLabelSize = 0, showRatioPie = F)
dev.off()
...
ggplot(data, aes(fill=Raza, y=Proporcion, x=`Etnia o tipo de muestra`)) +
  geom_bar(position="fill", stat="identity") +
  ggtitle("Distribucion de Raza, segun etnia y tipo de muestra") +
...
colnames(data) <- c("Etapa o tipo de muestra", "Diagnostico", "Proporcion")
data <- as.data.frame(merge(data, Total, by = "Etapa o tipo de muestra"))

pdf("df_plot4.pdf", height = 8.5, width = 13)
ggplot(data, aes(fill=Diagnostico, y=Proporcion, x=`Etapa o tipo de muestra`)) +
  geom_bar(position="fill", stat="identity") +
  ggtitle("Distribucion de diagnostico, segun Etapa y tipo de muestra") +
