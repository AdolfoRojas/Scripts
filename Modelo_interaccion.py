    ...
    for afected_gene in afected_genes_5_kb.ids:    
        try:
            ...
            if len(not_priorized.loc[not_priorized[0] == afected_gene,]) == 0:
                efecto_ponderado = efecto_ponderado * 1                     
            ...
            for interactor_1st_grade in asd:
                try:                    
                    if len(not_priorized.loc[not_priorized[0] == interactor_1st_grade,]) == 0:
                        efecto_ponderado += info2[interactor_1st_grade] * 0.5 * 1  
                    else:
                        efecto_ponderado += info2[interactor_1st_grade] * 0.5
                except KeyError:                
                    pass        
...
def Data_prep_1():
    ...
    result2 = result2.drop(columns={"Consequence"}).drop_duplicates()
...
def Evaluar_rendimiento2():
    ...
    info_snp.to_csv('../3er_Objetivo/info_snp_tab_INT_model.tsv', index=False, sep= "\t")
...
result3["Score_fix"] = (result3.Score)# * result3["Module"]  * (0.9 + 0.1 * result3["UP"]) * (0.9 + 0.1 * result3["DOWN"])
result3["Score2_fix"] = (result3.Score2)# * result3["Module"] *  (0.9 + 0.1 * result3["UP"]) * (0.9 + 0.1 * result3["DOWN"])
