...
Taruca_adolfo_tesis <- "adolfo@200.89.65.156:/media/run-projects/Adolfo/Resultados_Tesis/Objetivo_1/"
#################################################################################################
system("rm *.rds")
system("rm *.bk")
...
info <- read.delim("merged_imputed_PRS.list", header = FALSE)
#sumstats <- sumstats[sumstats$rsid%in% info$rsid,]
sumstats <- sumstats[sumstats$rsid%in% info$V1,]
...
for(semilla in 1:10){
  ...
  bot$sendMessage(chat_id, text = message_to_bot)
  set.seed(semilla)
  ind.test <- poblacion_restante[(1+150*(semilla-1)):(150+150*(semilla-1))]
...
system("scp -P 1313 Efectos_beta_validacion_cruzada.tsv adolfo@200.89.65.156:/media/run-projects/Adolfo/Resultados_Tesis/Objetivo_1/Efectos_beta_validacion_cruzada.tsv")
...
if (crossval_rendimiento[4] > crossval_rendimiento[2] & crossval_rendimiento[4] > crossval_rendimiento[3] & crossval_rendimiento[4] > crossval_rendimiento[5]) {
   best_auc_name <- "Grid SP model"
   best_auc_name2 <- "Grid_SP_model"
   best_auc_value <- AUC_pred_sp[[1]]
   best_auc_pred <- pred_sp
} else if (crossval_rendimiento[3] > crossval_rendimiento[2] & crossval_rendimiento[3] > crossval_rendimiento[4] & crossval_rendimiento[3] > crossval_rendimiento[5]) {
   best_auc_name <- "Grid NOSP model"
   best_auc_name2 <- "Grid_NOSP_model"
   best_auc_value <- AUC_pred_nosp[[1]]
   best_auc_pred <- pred_nosp
} else if (crossval_rendimiento[5] > crossval_rendimiento[2] & crossval_rendimiento[5] > crossval_rendimiento[3] & crossval_rendimiento[5] > crossval_rendimiento[4]) {
   best_auc_name <- "Automatic model"
   best_auc_name2 <- "Automatic_model"
   best_auc_value <- AUC_Auto[[1]]
   best_auc_pred <- final_pred_auto
} else if (crossval_rendimiento[2] > crossval_rendimiento[4] & crossval_rendimiento[2] > crossval_rendimiento[3] & crossval_rendimiento[2] > crossval_rendimiento[5]) {
...
system(paste("scp -P 1313 ", paste("VEP_p-Value_threshold_", p_value, "_hapmap3_all_variant_effect_non_zero_summary.html", sep = "")," ", Taruca_adolfo_tesis, paste("VEP_p-Value_threshold_", p_value, "_hapmap3_all_variant_effect_non_zero_summary.html", sep = ""), sep = ""))
system(paste("scp -P 1313 ", paste("VEP_p-Value_threshold_", p_value, "_hapmap3_all_variant_effect_summary.html", sep = "")," ", Taruca_adolfo_tesis, paste("VEP_p-Value_threshold_", p_value, "_hapmap3_all_variant_effect_summary.html", sep = ""), sep = ""))
...
system(paste("scp -P 1313 ", paste("VEP_p-Value_threshold_", p_value, "_hapmap3_all_variant_effect_non_zero", sep = "")," ", Taruca_adolfo_tesis, paste("VEP_p-Value_threshold_", p_value, "_hapmap3_all_variant_effect_non_zero", sep = ""), sep = ""))
system(paste("scp -P 1313 ", paste("VEP_p-Value_threshold_", p_value, "_hapmap3_all_variant_effect", sep = "")," ", Taruca_adolfo_tesis, paste("VEP_p-Value_threshold_", p_value, "_hapmap3_all_variant_effect", sep = ""), sep = ""))
...
less_risk <- head(Prediccion[order(Prediccion$best_auc_pred),],length(ind.final.val)*0.95)
low_risk <- head(Prediccion[order(Prediccion$best_auc_pred),],length(ind.final.val)*0.05)
odds_less_risk <- length(less_risk[less_risk$V2 == "Case",]$V2)/length(less_risk[less_risk$V2 == "Control",]$V2)
greater_risk <- tail(Prediccion[order(Prediccion$best_auc_pred),],length(ind.final.val)*0.05)
...
system(paste("scp -P 1313 ", paste("Density_plot_",best_auc_name2,"_p-Value_threshold_", p_value,".png", sep = "")," ", Taruca_adolfo_tesis, paste("Density_plot_",best_auc_name2,"_p-Value_threshold_", p_value,".png", sep = ""), sep = ""))
...
system(paste("scp -P 1313 ", paste("Scores_vs_samples_p-Value_", p_value,".png", sep = "")," ", Taruca_adolfo_tesis, paste("Scores_vs_samples_p-Value_", p_value,".png", sep = ""), sep = ""))
